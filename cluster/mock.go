...
type Mock struct {
	...
	UpdateDefinitionFunc     func(def []byte, res flux.ResourceID, container string, newImageID image.Ref) ([]byte, error)
...
func (m *Mock) UpdateDefinition(def []byte, res flux.ResourceID, container string, newImageID image.Ref) ([]byte, error) {
	return m.UpdateDefinitionFunc(def, res, container, newImageID)
}
...
func (m *Mock) UpdatePolicies(def []byte, res flux.ResourceID, p policy.Update) ([]byte, error) {
	return m.UpdatePoliciesFunc(def, res, p)
}
